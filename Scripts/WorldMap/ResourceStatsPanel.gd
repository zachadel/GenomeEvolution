...
#		res_tree[i][0].set_text(0, Settings.settings["resources"].keys()[i] + " : " + str(0) + " units")
...
#	for i in range(len(Settings.settings["resources"].keys())):
#		res_tree[i][0].set_text(0, Settings.settings["resources"].keys()[i] + " : " + str(resources[i]) + " units")
