...
func extend_with_blanks(cmsm_idx: int, num_blanks: int):
	STATS.set_final_blank_tiles(num_blanks) #this should work due to it updating, and fixes applying each time
	STATS.compare_max_blank_tiles(num_blanks)
...
func get_ate_list():
	if !ate_list_is_clean:
		...
		for g in get_all_genes():
			if (g.is_ate()):
				STATS.increment_currentTE(g.get_ate_personality_by_name())
				STATS.max_TE()
				ate_list.append(g);
		ate_list_is_clean = true;
	STATS.set_final_transposon_tiles(ate_list.count())
	STATS.compare_max_transposon_tiles(ate_list.count())
	return ate_list;
