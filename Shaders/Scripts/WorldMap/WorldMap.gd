...
func move_player(pos: Vector3):
	var player_tile = Game.world_to_map(current_player.position) #sets the current position
	var tiles_moved = 0 #says 0 tiles moved
	
	var loc_radius = current_player.organism.get_locomotion_radius() #recognizes the radius
	var vis_radius = current_player.organism.get_vision_radius() #shows what is visibile
	
	var distance = Game.get_distance_cubev(player_tile, pos) #figures out distance
	...
	STATS.increment_tiles_traveled(tiles_moved) #should add on how many tiles were moved, even if 0.
	return tiles_moved
